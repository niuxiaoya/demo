...
//  保障
  ...
  export default {
    ...
    methods:{
      openDetail(item) {
        this.$router.push(`/buy/detail?id=${item.gid}&uid=${item.cover_fid}`)
      },
//     价格商品 数据变化
      change(){
        switch (this.navIds){
...
//      价格请求
      getPrice(){
        ...
        this.loadings=true
        setTimeout(() => {
          this.$http.get(`${process.env.API.MARKET}/market/buyer/goodsList?price_l=${this.postData.money1*10000 || 0}&price_h=${this.postData.money2*10000|| -1}&rows=3&p=${this.price_p}`).then(res=>{
